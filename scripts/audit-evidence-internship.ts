...
import { evaluateRequirementsAgainstEvidence, retrieveTopRelevantEvidence, getCachedSemanticEmbedding } from "../lib/semanticMatcher";
...
async function runDetailedEvidenceAudit() {
  console.log("=================================================================");
  console.log("           DETAILED EVIDENCE AUDIT & PROVENANCE INSPECTION       ");
  console.log("=================================================================\n");

  // Step 1: Parse candidate resume into immutable CandidateEvidenceUnits
  const resume = await parseResume(AUTHENTIC_RACHIT_RESUME_TEXT);
  const evidenceUnits = resume.evidence_units || [];

  console.log("═════════════════════════════════════════════════════════════════");
  console.log(`1. RAW EXTRACTED CANDIDATE EVIDENCE UNITS (${evidenceUnits.length} Total Units)`);
  console.log("═════════════════════════════════════════════════════════════════");
  evidenceUnits.forEach((u) => {
    console.log(`[${u.id}] (${u.source_section}${u.source_title ? ` - ${u.source_title}` : ""})`);
    console.log(`  "${u.text}"`);
    console.log(`  Type: ${u.evidence_type}`);
  });
  console.log("\n");
  ...
  console.log("═════════════════════════════════════════════════════════════════");
  console.log(`2. EXTRACTED ATOMIC REQUIREMENTS FROM INTERNSHIP JD (${jd.requirements.length} Total)`);
  console.log("═════════════════════════════════════════════════════════════════");
  jd.requirements.forEach((r) => {
    console.log(`ID: ${r.id}`);
    console.log(`Name: ${r.name}`);
    console.log(`Category: ${r.category}`);
    console.log(`Criticality: ${r.criticality}`);
    console.log(`Importance: ${r.importance}`);
    console.log(`Type: ${r.requirement_type}`);
    console.log(`Description: ${r.description}`);
    console.log("-----------------------------------------------------------------");
  });
  console.log("\n");

  // Step 3: Audit Retrieval independently for each requirement
  console.log("═════════════════════════════════════════════════════════════════");
  console.log("3. INDEPENDENT EVIDENCE RETRIEVAL AUDIT");
  console.log("═════════════════════════════════════════════════════════════════");
  ...
  for (const req of jd.requirements) {
    const topRetrieved = await retrieveTopRelevantEvidence(req, evidenceUnits, evidenceEmbeddings, 5);
    console.log(`\nRequirement: [${req.id}] ${req.name} (${req.criticality})`);
    console.log(`Query Description: "${req.description}"`);
    console.log(`Top Retrieved Units (${topRetrieved.length}):`);
    topRetrieved.forEach((u, idx) => {
      console.log(`  ${idx + 1}. [${u.id}] (${u.source_section}) "${u.text.substring(0, 110)}..."`);
    });
  }
  console.log("\n");
  ...
  console.log("═════════════════════════════════════════════════════════════════");
  console.log("4. REQUIREMENT-BY-REQUIREMENT EVALUATION & SCORING");
  console.log("═════════════════════════════════════════════════════════════════");
  ...
  analysis.evaluations.forEach((ev) => {
    ...
    console.log(`Criticality: ${ev.criticality}`);
    console.log(`Weight: ${ev.importance === "required" ? 3.0 : ev.importance === "high" ? 2.0 : 1.0}`);
    console.log(`Match: ${ev.status}`);
    console.log(`Score: ${ev.score}`);
    console.log(`Evidence IDs: [${ev.evidence_ids.join(", ")}]`);
    console.log(`Exact Evidence: ${ev.evidence.map((e) => `"${e.text}"`).join(" | ") || "None"}`);
    console.log(`Evidence Strength: ${ev.evidence.length > 0 ? (ev.evidence.every((e) => e.evidence_id.startsWith("ev_skill_")) ? "skills_section_only" : "direct_experience_bullet") : "no_evidence"}`);
    console.log(`Confidence: ${ev.confidence}`);
    console.log(`Reasoning: ${ev.reasoning}`);
    console.log("-----------------------------------------------------------------");
  });

  // Step 5: Exact Mathematical Calculation Breakdown
  console.log("═════════════════════════════════════════════════════════════════");
  console.log("5. EXACT MATHEMATICAL SCORING AUDIT TABLE");
  console.log("═════════════════════════════════════════════════════════════════");
  console.log(`Requirement Name                              | Criticality | Weight | Score | Weighted Contribution`);
  console.log(`----------------------------------------------+-------------+--------+-------+----------------------`);
  ...
  for (const entry of analysis.audit_trail || []) {
    const weightedScore = entry.weight * entry.score * 100;
    totalNumerator += weightedScore;
    totalDenominator += entry.weight;
    const namePad = entry.requirement_name.padEnd(45, " ");
    const critPad = (entry.criticality || "soft").padEnd(11, " ");
    const weightPad = entry.weight.toString().padEnd(6, " ");
    const scorePad = entry.score.toString().padEnd(5, " ");
    const contribPad = `${entry.contribution_percent}%`;
    console.log(`${namePad} | ${critPad} | ${weightPad} | ${scorePad} | ${contribPad}`);
  }

  const calculatedOverall = totalDenominator > 0 ? totalNumerator / totalDenominator : 0;
  console.log(`----------------------------------------------+-------------+--------+-------+----------------------`);
  console.log(`Total Numerator   : ${totalNumerator.toFixed(2)}`);
  console.log(`Total Denominator : ${totalDenominator.toFixed(2)}`);
  console.log(`Calculated Score  : ${calculatedOverall.toFixed(2)}%`);
  console.log(`Displayed Score   : ${analysis.match_score}%`);
  console.log(`Hard Req Score    : ${analysis.hard_requirement_match_score}%`);
  console.log(`Pref Req Score    : ${analysis.preferred_requirement_match_score}%`);
  console.log(`Confidence Score  : ${analysis.confidence_score}% (${analysis.confidence_level.toUpperCase()})`);

  console.log("\n═════════════════════════════════════════════════════════════════");
  console.log("6. ELIGIBILITY CONSTRAINTS (SEPARATE FROM TECHNICAL SKILLS)");
  console.log("═════════════════════════════════════════════════════════════════");
  (analysis.eligibility_results || []).forEach((el) => {
    console.log(`Constraint: ${el.stated_requirement}`);
    console.log(`Status    : ${el.status}`);
    console.log(`Evidence  : "${el.candidate_evidence}"`);
    console.log(`Reasoning : ${el.reasoning}`);
    console.log("-----------------------------------------------------------------");
  });
}

runDetailedEvidenceAudit().catch((err) => {
